...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = async (email: string, password: string): Promise<{ error: string | null }> => {
    ...
    // Support both hashed and plain (for seeded demo) passwords
    const hashed = await sha256(password)
    const match = profile.password_hash === hashed || profile.password_hash === password
    if (!match) return { error: 'Invalid email or password' }

    // Build a simple signed-like JWT payload (stored in sessionStorage, not transmitted)
    const exp = Math.floor(Date.now() / 1000) + 60 * 60 * 8 // 8 hours
    const payload = btoa(JSON.stringify({ sub: profile.id, email: profile.email, role: profile.role, exp }))
    const token = `header.${payload}.sig`
    sessionStorage.setItem(ADMIN_KEY, token)
    setAdmin({ id: profile.id, email: profile.email, role: profile.role })
    return { error: null }
  }
