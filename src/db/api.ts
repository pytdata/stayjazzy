...
// ─── CHAT MESSAGES ────────────────────────────────────────────
export async function getChatMessages(bookingId: string): Promise<BookingChatMessage[]> {
...
export async function sendChatMessageObj(msg: Partial<BookingChatMessage>) {
...
export async function getPortfolioCategories() {
  ...
  return (data ?? []) as import('@/types/types').PortfolioCategory[]
}

export async function createPortfolioCategory(cat: Partial<import('@/types/types').PortfolioCategory>) {
  const { data, error } = await supabase.from('portfolio_categories').insert([cat]).select().single()
  if (error) throw error
  return data
}

export async function updatePortfolioCategory(id: string, updates: Partial<import('@/types/types').PortfolioCategory>) {
